...
function MorphingBlobs() {
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      />
      <motion.div
        className="absolute top-0 right-0 w-[500px] h-[500px] bg-violet/5 blur-[100px]"
        animate={{
          x: [0, 50, 0],
          y: [0, 30, 0],
          borderRadius: [
      ...
      {[...Array(20)].map((_, i) => (
        <motion.div
          ...
          style={{
            ...
            top: `${Math.random() * 100}%`,
            backgroundColor: i % 2 === 0 ? "rgba(15, 191, 154, 0.4)" : "rgba(108, 99, 255, 0.4)",
          }}
          animate={{
            y: [0, -30, 0],
...
function AnimatedLogo({ name, index }: { name: string; index: number }) {
  const [isHovered, setIsHovered] = useState(false)
  ...
  return (
    ...
    >
      <motion.span
        ...
        animate={{
          color: isHovered ? "#F7F9FC" : "#4B5563",
          textShadow: isHovered ? "0 0 20px rgba(15, 191, 154, 0.5)" : "none",
        }}
  ...
  return (
    <section className="py-24 bg-charcoal relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8" ref={ref}>
        ...
        >
          <motion.p
            className="text-emerald text-sm font-semibold tracking-wide uppercase mb-2"
            initial={{ opacity: 0 }}
        ...
        >
          ...
          {/* Infinite scroll logos - dual direction */}
          <div className="relative overflow-hidden">
            ...
            >
              {[...logos, ...logos, ...logos, ...logos].map((logo, index) => (
                <AnimatedLogo key={`row1-${index}`} name={logo} index={index} />
              ))}
            </motion.div>
            ...
            >
              {[...logos.reverse(), ...logos, ...logos, ...logos].map((logo, index) => (
                <AnimatedLogo key={`row2-${index}`} name={logo} index={index} />
              ))}
            </motion.div>
          </div>
        </motion.div>
