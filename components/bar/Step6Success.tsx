...
const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL;
const POLL_MS     = 5000;
const TIMEOUT_MS  = 720000; // 12 minutes
...
export default function Step6Success({ data }: Props) {
  ...
  const pollRef    = useRef<ReturnType<typeof setInterval> | null>(null);
  const countRef   = useRef<ReturnType<typeof setInterval> | null>(null);
  const markedDone = useRef(false); // only call once
  ...
  // ── Mark onboarding step=6 so dashboard redirect guard doesn't loop ──
  const markOnboardingComplete = useCallback(async () => {
    if (markedDone.current || !planId) return;
    ...
    try {
      ...
      await fetch(`${BACKEND_URL}/aeo/onboarding-step`, {
        ...
        body:    JSON.stringify({ planId, step: 6 }),
      });

      console.log("[Step6] onboarding_step set to 6 — dashboard redirect will now work");
    } catch (err) {
      console.error("[Step6] markOnboardingComplete failed:", err);
  ...
  const poll = useCallback(async () => {
    ...
    try {
      ...
      setStatus(ps);
      setProgress(prev => STATUS_PROGRESS[ps ?? ""] ?? prev);

      if (ps === "completed") {
        stopPolling();
        await markOnboardingComplete(); // step=6 BEFORE redirect
        startCountdown();
      }

      if (ps === "error" || ps === "failed") {
        stopPolling();
        await markOnboardingComplete(); // unblock dashboard on error too
      }
    } catch (err) {
      console.error("[Step6] poll error:", err);
    }
  }, [planId, markOnboardingComplete, startCountdown, stopPolling]);
  ...
  useEffect(() => {
    ...
    // Mark complete immediately — user has reached Step 6,
    // so even if they close the tab the dashboard won't loop them back
