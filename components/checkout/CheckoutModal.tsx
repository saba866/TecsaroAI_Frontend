...
interface PlanData {
  ...
  yearly_price:     number
  yearly_per_month: number   // price_yearly_monthly from DB
  monthly_plan_key: string
  yearly_plan_key:  string
  features:         string[]
  is_popular:       boolean
  trial_days:       number
  // display strings from DB (e.g. "₹2,999")
...
export function CheckoutModal({
  ...
  // Fetch plan pricing from backend on mount
  useEffect(() => {
    if (!mounted) return
  ...
  }, [plan, interval, plans, onClose, onSuccess])
          ...
          COMING SOON
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-bold text-white font-mono">{px_price}</p>
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/8 p-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm font-semibold text-white mt-0.5">
                  {selectedPlan?.name} · {price}{period}
                </p>
              </div>
