...
//   CreditCard, FolderKanban, Plus, Check, Loader2, AlertCircle,
// } from "lucide-react"
...
// // ─────────────────────────────────────────────────────────────────
// // TYPES — mirror public.profiles schema exactly
// // ─────────────────────────────────────────────────────────────────

// interface UserProfile {
//   name:       string
//   email:      string
//   avatar:     string   // single letter e.g. "S"  — column: avatar
//   tier:       string   // "starter" | "pro"        — column: tier
//   planLabel:  string
//   planPrice:  string
// }
...
//   { name: "Competitors", href: "/dashboard/competitors", icon: Users2, badge: 3 },
// ]
...
// // Maps the exact profiles row returned by GET /profile
// function buildUserProfile(data: any): UserProfile {
//   const name  = data?.name  ?? ""
//   const email = data?.email ?? ""
//   const tier  = data?.tier  ?? "starter"  // ← correct column name
...
//     planLabel: tier === "pro" ? "Pro Plan"   : "Starter Plan",
//     planPrice: tier === "pro" ? "₹24,900/mo" : "₹7,900/mo",
//   }
// }
...
//   // Returns { id, name, email, avatar, tier, ... }
//   // ─────────────────────────────────────────────────────────────
...
//     } catch (err) {
//       console.error("[Sidebar] fetchProfile:", err)
//     }
//   }

//   // ─────────────────────────────────────────────────────────────
//   // FETCH PROJECTS — GET /plans
//   // ─────────────────────────────────────────────────────────────
...
//         setLoadError(j?.error ?? `Failed to load (${res.status})`)
//         return
//       }
//       const json = await res.json()
...
//     return <p className="px-3 pt-4 pb-1 text-[10px] font-semibold tracking-widest uppercase text-white/30">{label}</p>
//   }
...
//                       <span className="text-[10px] text-white/30 font-mono truncate block">{activeProject.domain}</span>
//                     )}
//                   </div>
...
//                   <p className="text-[10px] text-red-500 font-mono font-semibold">⚠ {loadError}</p>
//                   <p className="text-[10px] text-red-400 mt-0.5">Check: NEXT_PUBLIC_BACKEND_URL, auth middleware, GET /plans</p>
//                 </div>
//               )}
...
//                         className={cn("flex items-center gap-2.5 w-full px-3 py-2.5 text-left transition-colors",
//                           isActive ? "bg-emerald/[0.07]" : "hover:bg-gray-50")}>
//                         <div className={cn("w-7 h-7 rounded-md flex items-center justify-center text-[11px] font-bold shrink-0",
//                           isActive ? "bg-emerald/20 text-emerald" : "bg-gray-100 text-gray-500")}>
//                           {p.name.charAt(0).toUpperCase()}
//                         </div>
//                         <div className="flex-1 min-w-0">
//                           <p className={cn("text-xs font-semibold truncate", isActive ? "text-emerald" : "text-gray-800")}>{p.name}</p>
...
//         {/* Plan chip — reads tier from profile */}
//         {!collapsed && userProfile && (
//           <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-lg px-3 py-2">
//             <div>
//               <p className="text-xs font-semibold text-white leading-tight">{userProfile.planLabel}</p>
//               <p className="text-[10px] text-white/40 font-mono">{userProfile.planPrice}</p>
//             </div>
//             {userProfile.tier !== "pro" && (
//               <Link href="/billing" className="text-[10px] font-semibold text-emerald hover:text-emerald/80 transition-colors">
//                 Upgrade
//               </Link>
//             )}
//           </div>
...
//         {/* User row — shows avatar initial from DB, name, email */}
...
//             {/* avatar column from profiles — single letter e.g. "S" */}
...
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import { useSidebar } from "@/components/dashboard/sidebar-context" // ✅ import context
...
export function DashboardSidebar() {
  ...
  // ✅ collapsed state now comes from shared context — layout reads same value
  ...
  const fetchProjects = async (token: string) => {
    ...
    try {
      ...
      if (!res.ok) { const j = await res.json().catch(() => ({})); setLoadError(j?.error ?? `Failed (${res.status})`); return }
      const json    = await res.json()
      const rows: any[] = Array.isArray(json) ? json : json?.data ?? json?.plans ?? []
      const mapped  = rows.map(mapProject)
      setProjects(mapped)
      const savedId = typeof window !== "undefined" ? localStorage.getItem("activeProjectId") : null
      setActiveProject(mapped.find(p => p.id === savedId) ?? mapped[0] ?? null)
    } catch (err) { console.error("[Sidebar] fetchProjects:", err); setLoadError("Network error") }
    ...
    return (
      <Link href={item.href} className={cn(
        ...
        {!collapsed && (
          <>
            <span className="flex-1">{item.name}</span>
            {item.badge ? <span className="ml-auto bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full leading-none">{item.badge}</span> : null}
          </>
        )}
        {collapsed && item.badge ? (
          <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full">{item.badge}</span>
        ) : null}
      </Link>
    )
  }
      ...
      {!collapsed && (
        <div className="mx-3 mt-3 shrink-0 relative" ref={dropdownRef}>
          <button onClick={() => setDropdownOpen(v => !v)}
            ...
            <p className="text-[9px] font-semibold tracking-widest uppercase text-white/30 mb-0.5">Active Project</p>
            <div className="flex items-center justify-between gap-2">
              {projectsLoading ? <Loader2 className="h-3.5 w-3.5 text-white/30 animate-spin" />
              : loadError ? (
                <div className="flex items-center gap-1.5 flex-1">
                  <AlertCircle className="h-3.5 w-3.5 text-red-400 shrink-0" />
                  <span className="text-xs text-red-400 truncate">Error loading</span>
                ...
                <>
                  <div className="min-w-0 flex-1">
                    <span className="text-sm font-semibold text-white block truncate leading-tight">{activeProject?.name ?? "No projects yet"}</span>
                    {activeProject?.domain && <span className="text-[10px] text-white/30 font-mono truncate block">{activeProject.domain}</span>}
                  </div>
                  <ChevronDown className={cn("h-3.5 w-3.5 text-white/30 group-hover:text-white/60 transition-transform duration-200 shrink-0", dropdownOpen && "rotate-180")} />
                </>
              )}
            </div>
          </button>
              ...
              {projects.length > 0 ? (
                <div className="py-1 max-h-48 overflow-y-auto">
                  {projects.map((p) => {
                    ...
                    return (
                      <button key={p.id} onClick={() => selectProject(p)}
                        className={cn("flex items-center gap-2.5 w-full px-3 py-2.5 text-left transition-colors", isActive ? "bg-emerald/[0.07]" : "hover:bg-gray-50")}>
                        <div className={cn("w-7 h-7 rounded-md flex items-center justify-center text-[11px] font-bold shrink-0", isActive ? "bg-emerald/20 text-emerald" : "bg-gray-100 text-gray-500")}>
                          {p.name.charAt(0).toUpperCase()}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={cn("text-xs font-semibold truncate", isActive ? "text-emerald" : "text-gray-800")}>{p.name}</p>
                ...
                <div className="px-3 py-5 text-center">
                  <p className="text-xs text-gray-500 font-medium mb-1">No projects found</p>
                  <p className="text-[11px] text-gray-400">{loadError ? "Could not load — check console" : "Create your first project below"}</p>
                </div>
              )}
              <div className="border-t border-gray-100">
                <Link href="/dashboard/projects" onClick={() => setDropdownOpen(false)}
      ...
      <div className="px-3 pb-4 pt-3 border-t border-white/10 space-y-2 shrink-0">
        {!collapsed && userProfile && (
          <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-lg px-3 py-2">
            ...
            {userProfile.tier !== "pro" && (
              <Link href="./billing" className="text-[10px] font-semibold text-emerald hover:text-emerald/80 transition-colors">Upgrade</Link>
            )}
          </div>
        )}
        <div className="flex items-center gap-2.5">
          ...
          {!collapsed && userProfile && (
            <div className="flex-1 min-w-0">
              {userProfile.name && <p className="text-xs font-semibold text-white truncate leading-tight">{userProfile.name}</p>}
              <p className="text-[10px] text-white/40 truncate font-mono">{userProfile.email}</p>
            </div>
          )}
          {!collapsed && (
            <button onClick={handleLogout} className="shrink-0 p-1.5 rounded-md text-white/30 hover:text-white hover:bg-white/10 transition-colors" title="Log out">
              <LogOut className="h-3.5 w-3.5" />
            </button>
          )}
        </div>
        {collapsed && (
          <button onClick={handleLogout} className="flex items-center justify-center w-full py-2 rounded-lg text-white/30 hover:text-white hover:bg-white/5 transition-colors" title="Log out">
            <LogOut className="h-4 w-4" />
          </button>
        )}
      </div>
