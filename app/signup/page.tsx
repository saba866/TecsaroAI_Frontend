...
// const isStarter    = planParam === "starter"
...
//     provider: "google",
//     options: {
//       redirectTo: `${origin}/auth/callback?plan=${plan}`,
//     },
//   })
// }
//   return (
...
//               {isPro ? "Pro Plan — free forever" : "free forever · No credit card"}
//             </span>
//           </div>
...
//             {(isPro ? [
//               { title: "Full Pro ",        desc: "3 brands, 50 prompts tracked, Perplexity included"            },
...
//   ) : (
//     <>
//       Get Started {isPro ? "Pro" : isStarter ? "Starter" : "Free"}
...
function SignupForm() {
  ...
  // ── Derive plan string ──
  const plan      = (["starter", "pro"].includes(planParam) ? planParam : "free") as "free" | "starter" | "pro"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user && data.session) {
        const fullName = `${form.firstName} ${form.lastName}`.trim()
        await syncProfileAfterSignup(data.session.access_token, fullName, plan)
        window.location.href = "/onboarding"
        return
      }
  ...
  // ── Left panel features by plan ──
  const features = isPro ? [
    { title: "Full Pro Access",                   desc: "3 brands, 50 prompts tracked, Perplexity included"             },
    { title: "Daily AI visibility reports",       desc: "Fresh data every morning across ChatGPT, Gemini & Perplexity"  },
    { title: "AI Citation tracking",              desc: "See which sources AI engines cite in your category"             },
    { title: "Advanced recommendations",          desc: "Priority support and exportable reports included"               },
  ] : isStarter ? [
    { title: "ChatGPT + Gemini tracking",         desc: "20 prompts tracked across 2 AI engines daily"                  },
    { title: "Daily AI visibility reports",       desc: "Fresh data every morning, fully automated"                      },
    { title: "Competitor detection & AEO Score",  desc: "See who beats you in AI answers and exactly how to fix it"     },
    { title: "Full technical audit",              desc: "All pages audited for AI visibility issues"                     },
  ] : [
    { title: "Free forever — no credit card",     desc: "Track your brand in Gemini with 10 prompts, no expiry"         },
    { title: "AEO Visibility Score",              desc: "See how visible your brand is in AI answers"                    },
    { title: "Competitor detection",              desc: "Find out which brands AI recommends instead of you"             },
  ]

  const accentColor = isPro ? "violet" : "emerald"
  const badgeText   = isPro ? "Pro Plan" : isStarter ? "Starter Plan" : "Free Forever"
      ...
      <div className="hidden lg:flex lg:w-[52%] bg-charcoal relative overflow-hidden flex-col justify-between px-14 py-12">
        ...
        <div className="relative max-w-md">
          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border mb-6 ${
            isPro ? "bg-violet/10 border-violet/20" : "bg-emerald/10 border-emerald/20"
          }`}>
            <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${isPro ? "bg-violet" : "bg-emerald"}`} />
            <span className={`text-[11px] font-mono font-bold tracking-widest uppercase ${isPro ? "text-violet" : "text-emerald"}`}>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-12 lg:px-12 bg-white overflow-y-auto">
        <div className="mx-auto w-full max-w-sm">
          ...
          <div className="mb-6">
            ...
            {!isPro && !isStarter && (
              <p className="text-sm text-graphite">Free forever — no credit card needed</p>
            )}
          </div>
          ...
          {success && (
            <div className="mb-4 px-4 py-3 rounded-xl bg-emerald/5 border border-emerald/20 text-emerald text-sm">
              {success}
            </div>
          )}
